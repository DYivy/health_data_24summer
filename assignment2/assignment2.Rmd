...
Variable 1: `note_id`：int, Unique identifier for each note in the dataset.

Variable 2: `description`: chr, unstructured text data, Brief description or summary of the patients' medical case, with disease specification and complaints.

Variable 3: `medical_specialty`: chr, describing the medical specialty related to each note, such as "Allergy / Immunology" or "Bariatrics".

Variable 4: `sample_name `: chr, title of the medical case, indicating specific procedures or treatments.

Variable 5 `transcription`: chr,unstructured text data, transcription of the medical notes, including detailed information about the case.

Variable 6 `keywords`: chr, keywords related to the medical note, summarising useful information of the medical case.
...
`Surgery` has far more cases thann the other catogories, followed by `Consult`, `Cardiovascular`, `Orthopedic` and `Radiology`.
...
The plot is highly skewed, most of the word counts are near the origin, showing many words have low counts of unigram tokens. However, there are a few outliers showing a very high number of count of unique words, indicating there are some unigram tokens appear highly frequent.
...
This plot shows similar pattern to the plot of unigram tokens, also highly skewed. It also shows many words have low counts of unique bigram. But there are some highly repeated bigrams. And since we use bigram instead of unigram, there are much more "words".
...
```{r}
tokenized.data.sen <- analysis.data %>% tidytext::unnest_tokens(word, transcription, token = "sentences")
```

```{r}
tokenized.data.sen %>%
  group_by(medical_specialty) %>% 
  summarise(unique_sentences = n_distinct(sentences))
```

??I think the steps are correct, tokenize the data into "sentences", then count the number of unique sentences for the three medical field "Orthopedic", "Radiology", and "Surgery". But the result seems unreliable. The number in the three categories should show some difference.
...
A general lemmatizer probably won't work well for medical data because it may not detect the nuances in similar medical terms with different meanings. It could reduce distinct terms to the same root, causing confusion. Given the importance of accuracy in medical terminology, this is not acceptable.
...
The correlation plot between Orthopedics and Surgery shows more points lying on the diagonal line, with a noticeable concentration towards the Orthopedic region. This is because Orthopedics is a specialized branch of Surgery, and both fields involve surgical procedures. Consequently, there is a significant overlap in related terms between the two disciplines, reflecting their close relationship.

In contrast, the correlation plot between Orthopedics and Radiology displays more scattered points, with many lying away from the diagonal line. This indicates a lower correlation in medical terminology between these two specialties. The difference may be due to Radiology's focus on imaging and diagnostics.
...
lemma.freq <- lemmatized.data %>% 
  ...
  dplyr::mutate(proportion = n / sum(n)) %>%
  tidyr::pivot_wider(names_from = medical_specialty, values_from = proportion) %>%
  tidyr::pivot_longer(`Orthopedic`:`Radiology`, names_to = "medical_specialty", values_to = "proportion")
...
The plot comparing Surgery and Radiology shows that the points are scattered and largely fall far from the diagonal line. This suggests that there are not many common medical terms between these two categories. This is understandable since Surgery focuses on operations, while Radiology focuses on imaging and diagnostics.
...
```{r}
all.counts.tfidf %>% dplyr::group_by(medical_specialty) %>%
  dplyr::slice_max(order_by=tf_idf, n=10)
```

**8** Are there any lemmas that stand out in these lists? Why or why not?

There are some lemmas are noticable.
Let's see the top three used lemma in each category：

`Orthopedic`: "range motion"(139)， "carpal ligament"(85)， "transverse carpal"(81).

`Radiology`: "left ventricular" (50), "ejection fraction" (42), "myocardial perfusion"(33).

`Surgery`: "closed vicryl" (174), "anesthesia endotracheal" (168), "endotracheal anesthesia" (167).

These all seem to to be terms related to specific medical category.
What seem interesting is "anesthesia endotracheal" and "endotracheal anesthesia". These two terms have similar counts, just reverse the word order, not sure if they should be combined, or kept like this. If the numbers are added, then the combined term becomes the most common used lemma in surgery. This can be true, since endotracheal is a common procedure in surgery.

Additionally, there are same words shown in both `Orthopedic` and `Surgery`: "closed vicryl" and "dissection carried". It make sense since Orthopedic is subdivision of `Surgery`,and these terms are commonly used in surgical procedures.
...
```{r}
analysis.data %>% dplyr::select(medical_specialty, transcription) %>% dplyr::filter(stringr::str_detect(transcription, 'closed vicryl')) %>% dplyr::slice(1:160)
```

I tried to add the example number to see if both `Orthopedic` and `Surgery` will show up in the cases. When I increase the number to 160, the `Orthopedic` category appears. 
...
```{r}
emr.lda2 <- topicmodels::LDA(emr.dcm, k=6, control=list(seed=42))
emr.topics <- tidytext::tidy(emr.lda2, matrix='beta')
```
...
No matter we use 5 or 6 top LDA, the most top terms remain the same, just may show up with different frequencies in different LDA.


```{r}
specialty_gamma <- tidytext::tidy(emr.lda2, matrix='gamma')
...
specialty_gamma2 <- dplyr::left_join(specialty_gamma, note_id_specialty_mapping)
```
```{r}
specialty_gamma2 %>%
  dplyr::mutate(medical_specialty = reorder(medical_specialty, gamma * topic)) %>%
...
This time, the `Surgery`, `Orthopedic` don not fall in one topic again. In terms of `Surgery` and `Orthopedic`, as the topic in LDA increase, the terms are more separate. However, the `Radiology` don't show much difference as the topic increases. `Radiology` still have a domain topic, and one sub-domain topic.
